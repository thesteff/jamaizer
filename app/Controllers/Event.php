...
{
	public function create($groupSlug = 0){
        if(count($_POST) > 0) {
    ...
    public function update($eventSlug){
    ...
    public function delete($eventSlug){
        $eventModel = new EventModel();
        $event = $eventModel->getOneEventBySlug($eventSlug);
    ...
    public function viewEvents($groupSlug = null){
        if(isset($_SESSION['logged']) && $_SESSION['logged']){
            $memberId = $_SESSION['member']['id'];
        } else {
            $memberId = 0;
        ...
        $eventModel = new EventModel();
        $events = $eventModel->getEvents($groupSlug);
    ...
    public function header($eventSlug){
        $eventModel = new EventModel();
        $event = $eventModel->getOneEventBySlug($eventSlug);
        $data['event'] = $event;
        return $data;
    ...
    public function viewOneEvent($eventSlug){
        if(isset($_SESSION['logged']) && $_SESSION['logged']){
            $memberId = $_SESSION['member']['id'];
        } else {
            $memberId = 0;
        ...
        echo view('templates/header');
        echo view('event/header', $data);
        echo view('event/viewOne', $data);
        echo view('templates/footer');
    }

    public function members($eventSlug){
        if(isset($_SESSION['logged']) && $_SESSION['logged']){
            $memberId = $_SESSION['member']['id'];
        } else {
            $memberId = 0;
    ...
    public function acceptMemberInEvent(    $eventSlug, $memberId){
