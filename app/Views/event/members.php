<div class="m-2">
    <?php if(isset($group['is_admin']) && $group['is_admin']) : ?>    
        ...
        <?php foreach($eventRequests as $request) : ?>
            ...
            <a href="<?= site_url('group/').esc($group['slug'], 'url').'/event/'.$request['event_id'].$event['name'].'members/accept/'.$request['member_id'] ?>"  class="btn btn-outline-dark">Accepter dans le groupe</a>
            </div>
        </div>
        <?php endforeach ?>
    <?php endif ?>
